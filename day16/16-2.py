...
for x in range(len(tiles)):
    res = max(res, check(tiles, (x, 0, 1)))
    res = max(res, check(tiles, (x, len(tiles[0]) - 1, 3)))

for y in range(len(tiles[0])):
    res = max(res, check(tiles, (0, y, 2)))
    res = max(res, check(tiles, (len(tiles) - 1, y, 0)))
